'''
SEED-5 HDF5 builder
Filtering information: 1-45Hz bandpass filter + 50Hz notch filter, 200Hz downsampling.

HDF5 file structure:
- <subject's name>
    - eeg (num_channels, num_samples)
    - label (array of labels for each trial)
    - chOrder (num_channels,) (list of channel names)
    - trialEnd (list of trial end points in samples)
    - hFreq #45Hz (high cutoff frequency for bandpass filter)
    - trialStart (list of trial start points in samples)
    - lFreq #1Hz (low cutoff frequency for bandpass filter)
    - sFreq #200 Hz (sampling frequency)
    - score (array of emotional scores for each trial)
'''
import mne
import h5py
import argparse
...
# settings #
############
HFREQ = 45 # EEG filter high cutoff frequency
LFREQ = 1 # EEG filter low cutoff frequency
RSFREQ = 200 # Resampling frequency
# Trigger information
